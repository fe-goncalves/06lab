...
type UserProfile = { id: string; full_name: string | null; role: string; status: string; created_at: string; auth_user_id: string };
...
const ROLE_LABELS: Record<string, string> = { main: "Administrador", supporter: "Supporter", relator: "Relator" };

export default function UsuariosClient({ users: initialUsers, currentUserId, currentRole }: Props) {
  ...
  const canCreateSupporter = currentRole === "main";
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="flex flex-col gap-1">
              ...
              <select value={role} onChange={e => setRole(e.target.value)} className={ic} style={is}>
                {canCreateSupporter && <option value="supporter">Supporter</option>}
            ...
            <li key={u.id} className="flex items-center justify-between gap-4 rounded-lg border px-4 py-3"
              ...
              {u.id !== currentUserId && u.role !== "main" && (
                <button type="button" onClick={() => handleToggleStatus(u.id, u.status)}
                  ...
                  style={{ borderColor: "var(--color-border)", color: u.status === "active" ? "var(--color-danger)" : "var(--color-success)" }}>
