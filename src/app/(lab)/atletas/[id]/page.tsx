...
export default function AtletaPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [
      ...
      supabase.from("athlete_team_stints").select("id, team_id, started_at, ended_at, is_current, movement_type, teams(id, full_name, abbreviation, logo_url)").eq("athlete_id", id).order("started_at", { ascending: false }),
      ...
      <div className="border-b" style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-surface)" }}>
        <div className="px-8 pt-6 pb-0">
          ...
          <div className="mb-4 flex items-center gap-4">
            {displayPhoto ? (
              <img src={displayPhoto} alt="" className="h-14 w-14 shrink-0 rounded-full border object-cover" style={{ borderColor: "var(--color-border)" }} />
            ) : (
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-0.5">
                {positionAbbr && <span className="font-mono text-xs" style={{ color: "var(--color-brand)" }}>{positionAbbr}</span>}
          ...
          <div className="space-y-6">
            <div className="rounded-xl border overflow-hidden" style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-surface)" }}>
              <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: "var(--color-border)" }}>
                <h2 className="font-mono text-xs uppercase tracking-widest" style={{ color: "var(--color-text-secondary)" }}>Linha do tempo</h2>
                    ...
                    {editingStintId === stint.id ? (
                      <div className="px-5 py-4 space-y-3">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="font-medium text-sm" style={{ color: "var(--color-text-primary)" }}>{stint.teams?.full_name ?? "Equipe"}</span>
                        </div>
                      ...
                      <div className="flex items-center gap-4 px-5 py-3 group">
                        ...
                        {stint.teams?.logo_url ? (
                          <img src={stint.teams.logo_url} alt="" className="h-8 w-8 rounded border object-contain shrink-0" style={{ borderColor: "var(--color-border)" }} />
                        ) : (
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-sm" style={{ color: "var(--color-text-primary)" }}>{stint.teams?.full_name ?? "Equipe desconhecida"}</p>
          ...
          <div className="space-y-6">
            {editionStats.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="mt-2 font-mono text-sm" style={{ color: "#A6A6A6" }}>Nenhuma edição com dados registrados.</p>
              </div>
            ) : (
              <div className="rounded-xl border overflow-hidden" style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-surface)" }}>
                <div className="px-5 py-4 border-b" style={{ borderColor: "var(--color-border)" }}>
                  <h2 className="font-mono text-xs uppercase tracking-widest" style={{ color: "var(--color-text-secondary)" }}>Por edição</h2>
                </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {editionStats.map((stat, idx) => (
                        <tr key={stat.edition_id} style={{ borderTop: idx > 0 ? "1px solid var(--color-border)" : "none" }} className="hover:bg-[rgba(255,255,255,0.02)]">
                          ...
                          <td className="px-4 py-3 font-mono text-xs" style={{ color: "var(--color-text-secondary)" }}>{stat.competition_editions?.seasons?.name ?? "—"}</td>
