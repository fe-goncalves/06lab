...
export function EquipeLogoUpload({ value, onChange, existingUrl = null, disabled = false }: EquipeLogoUploadProps) {
  ...
  return (
    <div className={`${styles.logoUploadPanel} ${disabled ? styles.logoUploadPanelDisabled : ""}`}>
      ...
      <div className={styles.logoUploadPreview}>
        {preview ? (
          <>
            <img src={preview} alt="" className={styles.logoUploadImg} />
            {!disabled && (
              <button type="button" className={styles.logoUploadRemove} onClick={removeImage} aria-label="Remover logo">
      ...
      <div className={styles.logoUploadBody}>
        <span className={styles.fieldLabel}>Logo da equipe</span>
        <p className={styles.logoUploadHint}>
          {disabled ? "Sem logo — pool do sistema" : "PNG, JPG ou WebP · proporção 1:1 recomendada"}
        </p>
        {!disabled && (
