...
export default function PartidaClient({
  ...
  const s = { borderColor: "var(--color-border)", backgroundColor: "var(--color-background)", color: "var(--color-text-primary)" };
  ...
  async function handleSave() {
    ...
    setSaving(false);
    if ("error" in r) { setSaveFeedback(r.error); return; }
  ...
  async function handleAddAction() {
    ...
    if (actionType === "goal") {
      const a = actionTeamId === match.team_a_id;
      if (!isOwnGoal) { if (a) setScoreA(p => p + 1); else setScoreB(p => p + 1); }
      else { if (a) setScoreB(p => p + 1); else setScoreA(p => p + 1); }
    }
    const athlete = athletes.find((a: any) => a.id === actionAthleteId);
    setActions(prev => [...prev, { id: r.id, action_type: actionType, team_id: actionTeamId, period: actionPeriod, minute: actionMinute ? Number(actionMinute) : null, is_own_goal: isOwnGoal, goal_type: isOwnGoal ? "own_goal" : goalType, primary_athlete: athlete ?? null }]);
    setShowActionForm(false);
    setActionMinute(""); setActionAthleteId(""); setActionAssistId(""); setIsOwnGoal(false); setGoalType("normal");
  }
  ...
  async function handleDelete(actionId: string, aType: string, own: boolean, teamId: string) {
    ...
    if (aType === "goal") {
      const a = teamId === match.team_a_id;
      if (!own) { if (a) setScoreA(p => Math.max(0, p - 1)); else setScoreB(p => Math.max(0, p - 1)); }
      else { if (a) setScoreB(p => Math.max(0, p - 1)); else setScoreA(p => Math.max(0, p - 1)); }
    }
    setActions(prev => prev.filter((a: any) => a.id !== actionId));
  }

  const first = [...actions].filter(a => a.period === "first").sort((a, b) => (a.minute ?? 0) - (b.minute ?? 0));
  const second = [...actions].filter(a => a.period === "second").sort((a, b) => (a.minute ?? 0) - (b.minute ?? 0));

  return (
    <div className="p-6 md:p-8">
      <div className="mb-6 flex items-center gap-2 text-sm" style={{ color: "var(--color-text-secondary)" }}>
        <Link href={`/competicoes/${competitionId}/edicoes/${edicaoId}/fases/${faseId}`} style={{ color: "var(--color-text-secondary)" }}>
          ← {competitionName}
        </Link>
        <span>·</span>
        <span>{roundName}</span>
      ...
      <div className="mb-6 rounded-xl border p-6" style={{ backgroundColor: "var(--color-surface)", borderColor: "var(--color-border)" }}>
        <div className="flex items-center justify-center gap-8 mb-6">
          <div className="flex flex-col items-center gap-2 min-w-[100px]">
            ...
            <p className="text-sm font-medium text-center" style={{ color: "var(--color-text-primary)" }}>{teamA?.full_name ?? "A definir"}</p>
          </div>
          ...
          <div className="flex flex-col items-center gap-2 min-w-[100px]">
            ...
            <p className="text-sm font-medium text-center" style={{ color: "var(--color-text-primary)" }}>{teamB?.full_name ?? "A definir"}</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center justify-center gap-3">
          <select value={status} onChange={e => setStatus(e.target.value)} className={ic} style={s}>
            {Object.entries(STATUS_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
          </select>
          <select value={finishType} onChange={e => setFinishType(e.target.value)} className={ic} style={s}>
          ...
          {saveFeedback && <span className="text-xs" style={{ color: "var(--color-success)" }}>{saveFeedback}</span>}
        </div>
        {match.match_date && (
      ...
      <div className="rounded-xl border p-5" style={{ backgroundColor: "var(--color-surface)", borderColor: "var(--color-border)" }}>
          ...
          <div className="mb-4 rounded-lg border p-4 space-y-3" style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-background)" }}>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <label className="flex flex-col gap-1">
                ...
                <select value={actionType} onChange={e => { setActionType(e.target.value); setIsOwnGoal(false); }} className={ic} style={s}>
                  {Object.entries(ACTION_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                </select>
              </label>
              <label className="flex flex-col gap-1">
                <span className="text-xs" style={{ color: "var(--color-text-secondary)" }}>Equipe</span>
                <select value={actionTeamId} onChange={e => { setActionTeamId(e.target.value); setActionAthleteId(""); setActionAssistId(""); }} className={ic} style={s}>
              ...
              <label className="flex flex-col gap-1">
                ...
                <select value={actionPeriod} onChange={e => setActionPeriod(e.target.value)} className={ic} style={s}>
              ...
              {needsMinute && (
                <label className="flex flex-col gap-1">
                  ...
                  <input type="number" min={1} max={halfDuration * 2} value={actionMinute} onChange={e => setActionMinute(e.target.value)} className={ic} style={s} />
                </label>
              )}
              {needsAthlete && (
                <label className="flex flex-col gap-1">
                  <span className="text-xs" style={{ color: "var(--color-text-secondary)" }}>Atleta</span>
                  <select value={actionAthleteId} onChange={e => setActionAthleteId(e.target.value)} className={ic} style={s}>
                    <option value="">Selecione…</option>
                    {athletes.map((a: any) => <option key={a.id} value={a.id}>{a.full_name}{a.surname ? ` (${a.surname})` : ""}</option>)}
                  </select>
                </label>
              )}
              {isGoal && (
                <label className="flex flex-col gap-1">
                  ...
                  <select value={isOwnGoal ? "own_goal" : goalType} onChange={e => { if (e.target.value === "own_goal") { setIsOwnGoal(true); setGoalType("normal"); } else { setIsOwnGoal(false); setGoalType(e.target.value); } }} className={ic} style={s}>
                    <option value="normal">Normal</option>
              ...
              {isGoal && !isOwnGoal && (
                <label className="flex flex-col gap-1">
                  ...
                  <select value={actionAssistId} onChange={e => setActionAssistId(e.target.value)} className={ic} style={s}>
                    <option value="">Sem assistência</option>
                    {athletes.filter((a: any) => a.id !== actionAthleteId).map((a: any) => <option key={a.id} value={a.id}>{a.full_name}</option>)}
                  </select>
                </label>
              )}
              {needsMiss && (
                <label className="flex flex-col gap-1">
                  ...
                  <select value={missResult} onChange={e => setMissResult(e.target.value)} className={ic} style={s}>
        ...
        {actions.length === 0 ? (
          ...
          <div className="space-y-1">
            {first.length > 0 && (
              <>
                <p className="pt-2 pb-1 text-xs font-mono uppercase" style={{ color: "var(--color-text-secondary)" }}>1º Tempo</p>
                {first.map((a: any) => (
                  <ActionRow key={a.id} action={a} teamA={teamA} teamB={teamB} matchTeamAId={match.team_a_id} onDelete={() => handleDelete(a.id, a.action_type, a.is_own_goal, a.team_id)} />
                ))}
              </>
            )}
            {second.length > 0 && (
              <>
                <p className="pt-4 pb-1 text-xs font-mono uppercase" style={{ color: "var(--color-text-secondary)" }}>2º Tempo</p>
                {second.map((a: any) => (
                  <ActionRow key={a.id} action={a} teamA={teamA} teamB={teamB} matchTeamAId={match.team_a_id} onDelete={() => handleDelete(a.id, a.action_type, a.is_own_goal, a.team_id)} />
                ))}
              </>
...
function ActionRow({ action, teamA, teamB, matchTeamAId, onDelete }: { action: any; teamA: any; teamB: any; matchTeamAId: string; onDelete: () => void }) {
  const isTeamA = action.team_id === matchTeamAId;
  const abbr = isTeamA ? (teamA?.abbreviation ?? teamA?.full_name ?? "A") : (teamB?.abbreviation ?? teamB?.full_name ?? "B");
  const name = action.primary_athlete?.surname ?? action.primary_athlete?.full_name ?? "";
  return (
    <div className="flex items-center justify-between rounded-lg px-3 py-2 gap-3" style={{ backgroundColor: "var(--color-background)" }}>
      <div className="flex items-center gap-3 min-w-0">
        ...
        <span className="text-base shrink-0">{ACTION_ICONS[action.action_type] ?? "·"}</span>
        <span className="text-xs font-medium shrink-0" style={{ color: "var(--color-brand)" }}>{abbr}</span>
      ...
      <button type="button" onClick={onDelete} className="shrink-0 rounded border px-2 py-0.5 text-xs" style={{ borderColor: "var(--color-border)", color: "var(--color-danger)" }}>×</button>
    </div>
  );
}